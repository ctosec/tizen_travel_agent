...
export const usePaymentStore = create<PaymentState>((set) => ({
  ...
  createSession: async (data) => {
    ...
    try {
      ...
      const checkoutUrl = `${serverBase}/api/payments/checkout?orderId=${encodeURIComponent(orderId)}&amount=${data.amount}&orderName=${encodeURIComponent(data.orderName)}&method=${encodeURIComponent(easyPay)}`;
