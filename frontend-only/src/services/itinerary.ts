...
const cache = new Map<string, { data: ItineraryDay[]; timestamp: number }>();
const CACHE_TTL = 10 * 60 * 1000;

export async function generateItinerary(input: GenerateItineraryInput): Promise<ItineraryDay[]> {
...
[
  {
    ...
    itinerary = await generateJSON<ItineraryDay[]>(prompt);
  } catch (error) {
    console.warn('Gemini itinerary generation failed, using fallback', error);
  ...
  for (const day of itinerary) {
    for (const activity of day.activities) {
      try {
        const places = await textSearch(`${activity.location} ${input.city}`, 1);
        if (places.length > 0 && places[0].photoRef) {
          activity.photoUrl = getPhotoUrl(places[0].photoRef, 400);
        } else {
  ...
  cache.set(cacheKey, { data: itinerary, timestamp: Date.now() });
  return itinerary;
}

function buildFallbackItinerary(input: GenerateItineraryInput): ItineraryDay[] {
